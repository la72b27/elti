...
// Filter to selected records via ?sel=1,3,5,... (_index values from main page)
let RECORDS = DATA.records;
try {
    const _sp = new URLSearchParams(location.search).get('sel');
    if (_sp) {
        const _set = new Set(_sp.split(',').map(Number).filter(n => n > 0));
        const _f = DATA.records.filter(r => _set.has(r._index));
        if (_f.length > 0) RECORDS = _f;
    }
} catch(e) {}
...
// Pick best geocoding result matching block + street (mirrors onemap_api.py)
function chooseBestResult(results, b, s) {
    if (!results || !results.length) return null;
    const ub = (b||'').replace(/^(BLK|BLOCK)\s*/i,'').trim().toUpperCase();
    const us = (s||'').trim().toUpperCase();
    let best = null, bestScore = -1;
    for (const item of results) {
        const lat = parseFloat(item.LATITUDE), lng = parseFloat(item.LONGITUDE);
        if (!isFinite(lat)||!isFinite(lng)||lat<=0||lng<=0) continue;
        const hasPostal = item.POSTAL && item.POSTAL !== 'NIL' && item.POSTAL !== '';
        const blk  = (item.BLK_NO   ||'').toUpperCase().trim();
        const road = (item.ROAD_NAME||'').toUpperCase().trim();
        let score = (hasPostal ? 100 : 0)
            + (ub && blk  === ub  ? 20 : 0)
            + (us && road === us  ? 20 : us && road && (road.includes(us)||us.includes(road)) ? 10 : 0);
        if (score > bestScore) { bestScore = score; best = [lat, lng]; }
    }
    return best;
}

async function _geoQuery(q, b, s) {
    ...
    try {
        ...
        return chooseBestResult(d.results||[], b, s);
    } catch { clearTimeout(timer); return null; }
}

async function geo(block, street) {
    // Normalize: strip BLK/BLOCK prefix (OneMap indexes HDB blocks without it)
    const b = (block||'').replace(/^(BLK|BLOCK)\s*/i,'').trim();
    const s = (street||'').trim();
    if (!b && !s) return null;
    const queries = b && s ? [b+' '+s, s] : [b||s];
    for (let i = 0; i < queries.length; i++) {
        const r = await _geoQuery(queries[i], b, s);
        if (r) return r;
...
<head>
    ...
    <style>
        ...
        .status-set { display:inline-block; background-color: #dc3545; color: #fff !important; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 0.9em; }
...
<body>
    <div class="container-fluid">
        ...
        <div class="table-container">
            <table class="table table-hover table-bordered">
                <thead>
                    <tr>
                        <th style="width:38px;text-align:center"><input type="checkbox" id="selectAll" title="全选/取消" style="width:15px;height:15px;cursor:pointer;vertical-align:middle"></th>
    ...
    <script>
        ...
        function render() {
            ...
            // Filter records
            filteredRecords = data.records.filter(row => row.RBE === currentRBE && (currentTC === 'ALL' || row.TC_Display === currentTC));
            ...
            filteredRecords.forEach(row => {
                ...
                cb.dataset.ridx = String(row._index);
                cb.checked = selectedSet.has(row._index);
                cb.onchange = () => {
                    if (cb.checked) selectedSet.add(row._index); else selectedSet.delete(row._index);
                    _syncSelectAll();
                };
                cbTd.appendChild(cb);
                tr.appendChild(cbTd);
                ...
                span.textContent = row.Status || 'SET';
        ...
        document.getElementById('routeBtn').onclick = () => {
            if (selectedSet.size === 0) { alert('请先勾选需要标记在地图上的条目'); return; }
            const indices = [...selectedSet].join(',');
            window.open('/route?sel=' + encodeURIComponent(indices), '_blank');
        };
        document.getElementById('selectAll').addEventListener('change', function() {
            document.querySelectorAll('.row-select').forEach(cb => {
                cb.checked = this.checked;
                const idx = parseInt(cb.dataset.ridx);
