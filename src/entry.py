...
ONEMAP_EMAIL = "qi.liu@softgridinc.com"
ONEMAP_PASSWORD = "S@ftgr1dSensing"
...
async function getToken(refresh){
  ...
  try{
    const r=await fetch(refresh?'/api/token?refresh=1':'/api/token',{signal:ctrl.signal});
    clearTimeout(tid);
...
async function fetchOm(pc,tok){
  ...
  try{
    return await fetch(
      ...
      {headers:{Authorization:tok},signal:ctrl.signal}
    );
...
async function fp(pc){
  try{
    ...
    if(res.status===401){token=await getToken(true);res=await fetchOm(pc,token);}
    if(res.ok){
...
async def _get_onemap_token(env, force=False):
    if not force:
        cached = await env.ELTI_DATA.get("onemap_token")
        if cached:
            return cached
    email = env.ONEMAP_EMAIL if hasattr(env, "ONEMAP_EMAIL") else ONEMAP_EMAIL
    password = env.ONEMAP_PASSWORD if hasattr(env, "ONEMAP_PASSWORD") else ONEMAP_PASSWORD
    init = Object.new()
    init.method = "POST"
    ...
    if token:
        try:
            await env.ELTI_DATA.put("onemap_token", token, expirationTtl=172800)
        except Exception:
            await env.ELTI_DATA.put("onemap_token", token)
    return token
...
async def on_fetch(request, env):
    try:
        ...
        if method == "GET" and "/api/token" in url:
            ...
            tok = await _get_onemap_token(env, force=force)
            return Response.new(
