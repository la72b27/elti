...
async def _d1_load_block(env, tc: str, pfx: str, block: str,
    ...
    try:
        res_lt = await env.elti_db.prepare(
            ...
            "lift_names_all, interface, lss, lmd_device_id "
    ...
    try:
        ...
        if eff_pc:
            res_lss = await env.elti_db.prepare(
                "SELECT lmd_ip, proxy_ip, vp_tun_ip, lmd_tun_ip, dvr_ip "
    ...
    # Build per-device list: one entry per LT row (each may have its own lmd_device_id/lss)
    lmd_devices = [
        {
    ...
    try:
        ...
        if method == "POST" and "/api/lss/upload" in url:
            ...
            sql = (
                ...
                "(postal_code, lmd_ip, proxy_ip, vp_tun_ip, lmd_tun_ip, dvr_ip) "
                "VALUES (?, ?, ?, ?, ?, ?)"
            )
            stmts = [
                env.elti_db.prepare(sql).bind(
