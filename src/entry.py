...
<body>
    ...
    <script>
        ...
        function render() {
            ...
            let tcHtml = `<button class="btn btn-tc ${currentTC === 'ALL' ? 'active' : ''}" onclick="setTC('ALL')">ALL</button>`;
            Object.keys(stats).sort().forEach(tc => {
                tcHtml += `<button class="btn btn-tc ${currentTC === tc ? 'active' : ''}" onclick="setTC('${tc}')">${tc} ${stats[tc]}</button>`;
            });
            ...
            filteredRecords.forEach(row => {
                ...
                // Construct row HTML
                tr.innerHTML = `
                    <td class="text-center row-index">${visibleCount}</td>
                    <td>${row.TC_Display}</td>
                    <td>${row.Pfx}</td>
                    <td>${row.Block}</td>
                    <td>${row.Lift}</td>
                    <td title="${row.Address}">
                        <a href="#" class="addr-link" style="text-decoration:none;color:inherit;border-bottom:1px dashed #666;cursor:pointer;">${row.Address}</a>
                    </td>
                    <td>${row.LCOY}</td>
                    <td>${fmtDate(row['Status Date'])}</td>
                    <td>${row.RBE_Display}</td>
                    <td><span class="status-set">${row.Status}</span></td>
                `;
                
                // Attach event listener to the link
                const link = tr.querySelector('.addr-link');
                link.onclick = (e) => {
                    e.preventDefault();
                    openOneMap(row.Block, row.Address, link);
                };
        ...
        async function openOneMap(block, address, linkEl) {
            ...
            try {
                ...
                if (data.results && data.results.length > 0) {
                    ...
                    const mapUrl = `https://www.onemap.gov.sg/main/v2/?lat=${lat}&lng=${lng}&zoomLevel=18&marker=${lat},${lng},${encodeURIComponent(block + ' ' + address)}`;
        ...
        function setTC(tc) { currentTC = tc; render(); }
...
async def on_fetch(request, env):
    try:
        ...
        # 修复：使用 replace 替换占位符，避免 Python 字符串拼接错误
        html = HTML_TEMPLATE.replace("{{DATA_JSON}}", data_json)
        html = html.replace("{{LOGO_BASE64}}", LOGO_BASE64)
        
        return Response.new(html, headers=Headers.new([["Content-Type", "text/html"]]))
