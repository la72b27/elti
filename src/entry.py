    ...
    try:
        ...
        if method == "POST" and "/api/mask" in url:
            ...
            if not isinstance(mask_data, list):
                return Response.new(json.dumps({"error": "Invalid format"}), status=400, headers=Headers.new([["Content-Type", "application/json"]]))
            # Sanitize: strings only, max 10 chars each, max 2000 entries
            clean = [str(p).strip()[:10] for p in mask_data if isinstance(p, (str, int)) and str(p).strip()][:2000]
            await env.ELTI_DATA.put("mask_data", json.dumps(clean))
            return Response.new(json.dumps({"saved": len(clean)}), headers=Headers.new([["Content-Type", "application/json"]]))
        ...
        # Escape < > & to prevent </script> injection in embedded JSON blocks
        def _safe(s):
            return s.replace("&", "\\u0026").replace("<", "\\u003c").replace(">", "\\u003e")

        safe_json = _safe(data_json)
        safe_mask = _safe(json.dumps(mask_list))

        html = HTML_TEMPLATE.replace("{{DATA_JSON}}", safe_json)
        html = html.replace("{{MASK_JSON}}", safe_mask)
        ...
        print(f"[error] {e}")
        return Response.new(json.dumps({"error": "Internal server error"}), status=500, headers=Headers.new([["Content-Type", "application/json"]]))
