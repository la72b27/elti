...
async function fp(pc){
  try{
    const res=await fetch("https://www.onemap.gov.sg/api/common/elastic/search?searchVal="+encodeURIComponent(pc)+"&returnGeom=Y&getAddrDetails=Y&pageNum=1");
    if(!res.ok){fail++;return;}
    const j=await res.json();
    if(!Array.isArray(j.results)||!j.results.length){fail++;return;}
    const r0=j.results[0];
    const lat=parseFloat(r0.LATITUDE),lng=parseFloat(r0.LONGITUDE);
    if(!isFinite(lat)||!isFinite(lng)){fail++;return;}
...
<\/script>
        ...
        function openRouteMap() {
            ...
            if (win) { win.document.write(buildMapHtml(allRec)); win.document.close(); }
        }
        render();
    </script>
</body>
