...
<head>
    ...
    <style>
        ...
        .btn-search-toggle { border-color: rgb(45,0,125); color: rgb(45,0,125); }
        .btn-search-toggle:hover { background-color: rgb(45,0,125); color: #fff; border-color: rgb(45,0,125); }
...
async def _d1_load_block(env, tc: str, pfx: str, block: str,
    ...
    try:
        res_lt = await env.elti_db.prepare(
            ...
            "FROM masterlist_lt WHERE tc = ? AND pfx = ? AND block = ? LIMIT 1"
        ).bind(tc, pfx, block).all()
        ...
        if lt_rows:
            ...
            if pc_for_lt:
                res_lt2 = await env.elti_db.prepare(
                    ...
                    "town_council, full_add, postal_code AS lt_postal_code, "
                    "lift_names_all, interface, lss, lmd_device_id "
                    "FROM masterlist_lt WHERE postal_code = ? AND postal_code != '' LIMIT 1"
                ).bind(pc_for_lt).all()
                lt_rows2 = res_lt2.results if res_lt2.results else []
                if lt_rows2:
                    lt_row = lt_rows2[0]
    except Exception as e:
