...
async def _d1_load_block(env, tc: str, pfx: str, block: str) -> list:
    ...
    Uses two separate simple queries to avoid D1's correlated-subquery limitation.
    Primary: D1 (TMS records + LT enrichment).
    Fallback: KV cached_data (TMS fields only, no LT enrichment).
    """
    # ── Step 1: TMS records ──────────────────────────────────────────────────
    tms_rows = []
    try:
    ...
    if tms_rows:
        try:
            res_lt = await env.elti_db.prepare(
                "SELECT town_council, full_add, postal_code AS lt_postal_code, "
            ...
            if lt_rows:
                ...
                postcode = _dv(tms_rows[0], "postcode") or ""
                if postcode:
    ...
    # ── Step 2b: LSS enrichment (by TMS postcode) ────────────────────────────
    lss_row = None
    if tms_rows:
        try:
            tms_postcode_raw = _dv(tms_rows[0], "postcode") or ""
            if tms_postcode_raw:
