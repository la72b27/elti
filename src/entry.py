...
// Use localStorage selection when opened from main page with ?sel=1
let RECORDS = DATA.records;
if (new URLSearchParams(location.search).get('sel') === '1') {
    try {
        const _s = localStorage.getItem('elti_route_sel');
        if (_s) { RECORDS = JSON.parse(_s); localStorage.removeItem('elti_route_sel'); }
    } catch(e) {}
}
...
<body>
    ...
    <script>
        ...
        document.getElementById('routeBtn').onclick = () => {
            ...
            const sel = data.records.filter(r => selectedSet.has(r._index));
            localStorage.setItem('elti_route_sel', JSON.stringify(sel));
            window.open('/route?sel=1', '_blank');
        };
