...
<head>
    ...
    <style>
        ...
        input[type="radio"].exclude-radio { cursor:pointer; width:15px; height:15px; accent-color:#dc3545; vertical-align:middle; }
    </style>
</head>
<body>
    <div class="container-fluid">
        ...
        <div class="table-container">
            <table class="table table-hover table-bordered">
                <thead>
                    <tr>
                        <th><div class="header-text" title="Select postcode to exclude from Route Map" style="opacity:0.85;font-size:1.1em">○</div></th>
    ...
    <script>
        ...
        let excludedPostcode = localStorage.getItem(EXCLUDE_PC_KEY) || null;
        ...
        function render() {
            ...
            const restoredPostcodes = new Set();
            filteredRecords.forEach(row => {
                const tr = document.createElement('tr');
                tr.className = 'alarm-row';
                // Radio button — exclude this postcode from Route Map (persisted in localStorage)
                const pc = (row.Postcode || '').trim();
                const shouldCheck = !!excludedPostcode && pc === excludedPostcode && !restoredPostcodes.has(pc);
                if (shouldCheck) restoredPostcodes.add(pc);
                const radioTd = document.createElement('td');
                radioTd.className = 'text-center';
                const radio = document.createElement('input');
                radio.type = 'radio';
                radio.name = 'exclude-pc';
                radio.value = pc;
                radio.className = 'exclude-radio';
                radio.title = 'Select to exclude this postcode from Route Map';
                radio.checked = shouldCheck;
                let radioWasChecked = false;
                radio.addEventListener('mousedown', function() { radioWasChecked = this.checked; });
                radio.addEventListener('click', function() {
                    if (radioWasChecked) {
                        this.checked = false;
                        excludedPostcode = null;
                        localStorage.removeItem(EXCLUDE_PC_KEY);
                    } else {
                        excludedPostcode = pc;
                        localStorage.setItem(EXCLUDE_PC_KEY, pc);
                    }
                });
...
<\/script>
        ...
        function openRouteMap() {
            const allRec = data.records.filter(r => {
                ...
                if (excludedPostcode && pc === excludedPostcode) return false;
                return true;
            });
...
async def on_fetch(request, env):
    try:
        ...
        return Response.new(html, headers=Headers.new([
            ...
            ["Content-Security-Policy", "default-src 'self'; script-src 'unsafe-inline' https://cdn.jsdelivr.net; style-src 'unsafe-inline' https://cdn.jsdelivr.net; connect-src 'self' https://www.onemap.gov.sg https://cdn.jsdelivr.net; img-src 'self' data: https://*.tile.openstreetmap.org https://www.onemap.gov.sg; frame-ancestors 'none'"],
        ]))
