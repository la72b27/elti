...
<head>
    ...
    <style>
        ...
        @media (max-width: 576px) {
            ...
            .logo-container { justify-content: flex-start; flex-shrink: 0; }
            .logo-img { height: 56px; }
            .filter-info-inline { font-size: 0.6em; padding: 5px 8px; flex-grow: 0; flex-shrink: 1; min-width: 0; }
...
<body>
    <div class="container-fluid">
        ...
        <div class="header-btns">
            ...
            <button class="btn btn-stat btn-search-toggle" id="searchBtn">Search</button>
        </div>
    ...
    <script>
        ...
        function render() {
            ...
            filteredRecords.forEach((row, i) => {
                ...
                // Postcode — clickable link → /block_detail (new tab)
                const pcTd = document.createElement('td');
                const pcLink = document.createElement('a');
                pcLink.href = '/block_detail?tc=' + encodeURIComponent(row.TC_Display || '')
                            + '&pfx=' + encodeURIComponent(row.Pfx || '')
                            + '&block=' + encodeURIComponent(row.Block || '');
                pcLink.target = '_blank';
                pcLink.rel = 'noopener noreferrer';
                pcLink.style.cssText = 'text-decoration:none;color:inherit;border-bottom:1px dashed #aaa;cursor:pointer;';
                pcLink.textContent = row.Postcode ?? '';
                pcTd.appendChild(pcLink);
                tr.appendChild(pcTd);
        ...
        document.getElementById('searchBtn').onclick = () => { window.location.href = '/search'; };
...
def _filter_lift_ips(aggregated: str, linked_lifts: str) -> str:
    """Filter 'A=ip, B=ip, C=ip' to only entries whose lift name is in linked_lifts.

    linked_lifts: comma-separated lift names, e.g. 'A,B'
    Returns the filtered string, or the full aggregated string if linked_lifts is empty.
    """
    if not aggregated or not linked_lifts:
        return aggregated or ""
    wanted = {lf.strip().upper() for lf in linked_lifts.split(",") if lf.strip()}
    if not wanted:
        return aggregated
    parts = [
        p.strip() for p in aggregated.split(",")
        if "=" in p and p.strip().partition("=")[0].strip().upper() in wanted
    ]
    return ", ".join(parts)
...
async def _d1_load_block(env, tc: str, pfx: str, block: str,
                         postcode: str = "") -> list:
    """Return row dicts for one block (both RBEs) with LT + LSS enrichment.

    When no TMS alarm records exist but a masterlist_lt entry is found, returns
    a single stub row (rbe='') so block_detail can still render LT enrichment.

    postcode: optional caller-supplied postcode; used as a fallback for LT/LSS
    lookups when no TMS records are present.
    """
    # ── Step 1: TMS records ──────────────────────────────────────────────────
    tms_rows = []
    try:
        res = await env.elti_db.prepare(
            "SELECT tc, pfx, block, lift, address, postcode, lcoy, status_date, "
            "rbe, rbe_display, status "
            "FROM records WHERE tc = ? AND pfx = ? AND block = ? ORDER BY rbe"
        ).bind(tc, pfx, block).all()
        tms_rows = res.results if res.results else []
    except Exception as e:
        print(f"[block_detail d1 tms] {e}")

    # ── Step 2: LT enrichment (exact match, then postcode fallback) ──────────
    lt_rows = []
    try:
        res_lt = await env.elti_db.prepare(
            "SELECT tc AS lt_tc, pfx AS lt_pfx, block AS lt_block, "
            "town_council, full_add, postal_code AS lt_postal_code, "
            "lift_names_all, lift_name_linked, interface, lss, lmd_device_id, lmd_ip "
            "FROM masterlist_lt WHERE tc = ? AND pfx = ? AND block = ?"
        ).bind(tc, pfx, block).all()
        lt_rows = res_lt.results if res_lt.results else []
        if not lt_rows:
            # Postcode fallback: use TMS postcode or the caller-supplied one
            pc_for_lt = ((_dv(tms_rows[0], "postcode") or "") if tms_rows else "") or postcode
            if pc_for_lt:
                res_lt2 = await env.elti_db.prepare(
                    "SELECT tc AS lt_tc, pfx AS lt_pfx, block AS lt_block, "
                    "town_council, full_add, postal_code AS lt_postal_code, "
                    "lift_names_all, lift_name_linked, interface, lss, lmd_device_id, lmd_ip "
                    "FROM masterlist_lt WHERE postal_code = ? AND postal_code != ''"
                ).bind(pc_for_lt).all()
                lt_rows = res_lt2.results if res_lt2.results else []
    except Exception as e:
        print(f"[block_detail d1 lt] {e}")

    lt_row = lt_rows[0] if lt_rows else None

    # ── Step 2b: LSS enrichment ───────────────────────────────────────────────
    lss_row = None
    try:
        tms_pc = (_dv(tms_rows[0], "postcode") or "") if tms_rows else ""
        lt_pc  = (_dv(lt_row, "lt_postal_code") or "") if lt_row else ""
        eff_pc = tms_pc or lt_pc or postcode
        if eff_pc:
            res_lss = await env.elti_db.prepare(
                "SELECT lmd_ip, lmd_ips, proxy_ip, vp_tun_ip, lmd_tun_ip, dvr_ip "
                "FROM masterlist_lss WHERE postal_code = ? LIMIT 1"
            ).bind(eff_pc).all()
            lss_rows_r = res_lss.results if res_lss.results else []
            if lss_rows_r:
                lss_row = lss_rows_r[0]
    except Exception as e:
        print(f"[block_detail d1 lss] {e}")

    # ── Step 3a: Merge TMS + LT + LSS ────────────────────────────────────────
    # Build per-device list: one entry per LT row, with per-device filtered IPs
    lmd_devices = []
    for lt_r in lt_rows:
        linked   = _dv(lt_r, "lift_name_linked")
        raw_ips  = (_dv(lss_row, "lmd_ips") or "") if lss_row else ""
        raw_dvr  = (_dv(lss_row, "dvr_ip")  or "") if lss_row else ""
        # LMD IP: prefer per-device IP from masterlist_lt (populated from TMS LMD config)
        dev_lmd_ip = _dv(lt_r, "lmd_ip") or ""
        if not dev_lmd_ip:
            # Fall back to LSS-derived per-lift IPs, then primary lmd_ip
            dev_lmd_ip = _filter_lift_ips(raw_ips, linked)
            if not dev_lmd_ip:
                dev_lmd_ip = (_dv(lss_row, "lmd_ip") or "") if lss_row else ""
        lmd_devices.append({
            "lmd_device_id":    _dv(lt_r, "lmd_device_id"),
            "lift_name_linked": linked,
            "lss":              _dv(lt_r, "lss"),
            "lmd_ip":           dev_lmd_ip,
            "proxy_ip":         (_dv(lss_row, "proxy_ip")   or "") if lss_row else "",
            "vp_tun_ip":        (_dv(lss_row, "vp_tun_ip")  or "") if lss_row else "",
            "lmd_tun_ip":       (_dv(lss_row, "lmd_tun_ip") or "") if lss_row else "",
            "dvr_ip":           _filter_lift_ips(raw_dvr, linked) or raw_dvr,
        })

    result = []
    for row in tms_rows:
        tms_postcode = _dv(row, "postcode") or ""
        lt_postcode  = (_dv(lt_row, "lt_postal_code") or "") if lt_row else ""
        merged_postcode = lt_postcode if lt_postcode else tms_postcode
        result.append({
            "tc":             _dv(row, "tc"),
            "pfx":            _dv(row, "pfx"),
            "block":          _dv(row, "block"),
            "lift":           _dv(row, "lift"),
            "address":        _dv(row, "address"),
            "postcode":       merged_postcode,
            "lcoy":           _dv(row, "lcoy"),
            "status_date":    _dv(row, "status_date"),
            "rbe":            _dv(row, "rbe"),
            "rbe_display":    _dv(row, "rbe_display"),
            "status":         _dv(row, "status") or "1",
            "town_council":   (_dv(lt_row, "town_council")   or "") if lt_row else "",
            "full_add":       (_dv(lt_row, "full_add")        or "") if lt_row else "",
            "lt_postal_code": lt_postcode,
            "lift_names_all": (_dv(lt_row, "lift_names_all") or "") if lt_row else "",
            "interface":      (_dv(lt_row, "interface")       or "") if lt_row else "",
            "lss":              (_dv(lt_row, "lss")              or "") if lt_row else "",
            "lmd_device_id":    (_dv(lt_row, "lmd_device_id")   or "") if lt_row else "",
            "lift_name_linked": (_dv(lt_row, "lift_name_linked") or "") if lt_row else "",
            "lmd_ip":           (_dv(lss_row, "lmd_ip")    or "") if lss_row else "",
            "proxy_ip":         (_dv(lss_row, "proxy_ip")   or "") if lss_row else "",
            "vp_tun_ip":        (_dv(lss_row, "vp_tun_ip")  or "") if lss_row else "",
            "lmd_tun_ip":       (_dv(lss_row, "lmd_tun_ip") or "") if lss_row else "",
            "dvr_ip":           (_dv(lss_row, "dvr_ip")     or "") if lss_row else "",
            "lmd_devices":      lmd_devices,
        })

    # ── Step 3b: No TMS records but LT data found → synthesize stub row ───────
    if not result and lt_row:
        lt_postcode = _dv(lt_row, "lt_postal_code") or postcode
        result.append({
            "tc":             _dv(lt_row, "lt_tc") or tc,
            "pfx":            _dv(lt_row, "lt_pfx") or pfx,
            "block":          _dv(lt_row, "lt_block") or block,
            "lift":           "",
            "address":        _dv(lt_row, "full_add"),
            "postcode":       lt_postcode,
            "lcoy":           "",
            "status_date":    "",
            "rbe":            "",
            "rbe_display":    "",
            "status":         "0",
            "town_council":   _dv(lt_row, "town_council"),
            "full_add":       _dv(lt_row, "full_add"),
            "lt_postal_code": lt_postcode,
            "lift_names_all": _dv(lt_row, "lift_names_all"),
            "interface":      _dv(lt_row, "interface"),
            "lss":              _dv(lt_row, "lss"),
            "lmd_device_id":    _dv(lt_row, "lmd_device_id"),
            "lift_name_linked": _dv(lt_row, "lift_name_linked"),
            "lmd_ip":           (_dv(lss_row, "lmd_ip")    or "") if lss_row else "",
            "proxy_ip":         (_dv(lss_row, "proxy_ip")   or "") if lss_row else "",
            "vp_tun_ip":        (_dv(lss_row, "vp_tun_ip")  or "") if lss_row else "",
            "lmd_tun_ip":       (_dv(lss_row, "lmd_tun_ip") or "") if lss_row else "",
    ...
    # ── Step 4: KV fallback (no D1 TMS records and no LT stub) ──────────────
    if not result:
        try:
            stored = await env.ELTI_DATA.get("cached_data")
            if stored:
                kv_records = json.loads(stored).get("records", [])
                for r in kv_records:
                    if (r.get("TC_Display", "") == tc
                            and r.get("Pfx", "") == pfx
                        ...
                        result.append({
                            ...
                            "rbe":            r.get("RBE", ""),
                            "rbe_display":    r.get("RBE_Display", ""),
                            "status":         str(_status_int(r.get("Status", 1))),
                            "town_council":   "",
                            "full_add":       "",
                            "lt_postal_code": "",
                            "lift_names_all": "",
            ...
            print(f"[block_detail kv fallback] {e}")
...
async def on_fetch(request, env):
    try:
        ...
        if method == "GET" and "/block_detail" in url:
            ...
            tc_p    = params.get("tc",       "").strip().upper()
            pfx_p   = params.get("pfx",      "").strip().upper()
            block_p = params.get("block",    "").strip().upper()
            pc_p    = params.get("postcode", "").strip()

            # Postcode-only access: resolve tc/pfx/block from masterlist_lt
            if pc_p and not tc_p:
                try:
                    lt_anc = await env.elti_db.prepare(
                        "SELECT tc, pfx, block FROM masterlist_lt "
                        "WHERE postal_code = ? LIMIT 1"
                    ).bind(pc_p).first()
                    if lt_anc:
                        tc_p    = _dv(lt_anc, "tc")
                        pfx_p   = _dv(lt_anc, "pfx")
                        block_p = _dv(lt_anc, "block")
                except Exception as e:
                    print(f"[block_detail postcode lookup] {e}")

            # NS was renamed to YS – map any lingering NS input
            if tc_p == "NS":
                tc_p = "YS"
            ...
            last_updated_bd = ""
            try:
                rows = await _d1_load_block(env, tc_p, pfx_p, block_p, pc_p)
                meta = await env.elti_db.prepare(
                    "SELECT value FROM meta WHERE key = 'last_updated'"
                ).first()
                last_updated_bd = (_dv(meta, "value") or "") if meta else ""
            except Exception as e:
                print(f"[block_detail error] {e}")
            html = block_detail.render_html(rows, tc_p, pfx_p, block_p, last_updated_bd)
        ...
        if method == "POST" and "/api/lt/upload" in url:
            ...
            sql = (
                "INSERT INTO masterlist_lt "
                "(tc, pfx, block, town_council, full_add, postal_code, "
                " lift_names_all, lift_name_linked, interface, lss, lmd_device_id) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                "ON CONFLICT(tc, pfx, block, lmd_device_id) DO UPDATE SET "
                "town_council=excluded.town_council, full_add=excluded.full_add, "
                "postal_code=excluded.postal_code, lift_names_all=excluded.lift_names_all, "
                "lift_name_linked=excluded.lift_name_linked, interface=excluded.interface, "
                "lss=excluded.lss"
            )
            stmts = [
                env.elti_db.prepare(sql).bind(
                    ...
                    r.get("lift_names_all", ""),   r.get("lift_name_linked", ""),
                    r.get("interface", ""),        r.get("lss", ""),
                    r.get("lmd_device_id", ""),
                )
                for r in lt_records
        ...
        # ── POST /api/lss/upload  (LSS MasterList IP enrichment) ─────────────
        if method == "POST" and "/api/lss/upload" in url:
            auth_token     = request.headers.get("X-Update-Token")
            expected_token = env.UPDATE_TOKEN if hasattr(env, "UPDATE_TOKEN") else None
            if expected_token and auth_token != expected_token:
                return Response.new(json.dumps({"error": "Unauthorized"}), status=401,
            ...
            payload_str = await request.text()
            data        = json.loads(payload_str)
            lss_records = data.get("records", [])

            sql = (
                "INSERT OR REPLACE INTO masterlist_lss "
                "(postal_code, lmd_ip, lmd_ips, proxy_ip, vp_tun_ip, lmd_tun_ip, dvr_ip) "
                "VALUES (?, ?, ?, ?, ?, ?, ?)"
            )
            stmts = [
                env.elti_db.prepare(sql).bind(
                ...
                for r in lss_records
                if r.get("postal_code")
            ]
            for i in range(0, len(stmts), 100):
                await env.elti_db.batch(stmts[i:i + 100])

            print(f"[lss] upserted {len(stmts)} masterlist_lss rows")
            return Response.new(json.dumps({"upserted": len(stmts)}),
                                headers=Headers.new([["Content-Type", "application/json"]]))
        ...
        if method == "GET" and "/api/search" in url:
            ...
            # Resolve postcode → (tc, pfx, block) via masterlist_lt
            if pc_p and not tc_p:
                try:
                    lt_anc = await env.elti_db.prepare(
                        "SELECT tc, pfx, block FROM masterlist_lt "
            ...
            if not tc_p or not block_p:
                return Response.new(
                    json.dumps({"found": False,
                                "message": "No matching block found for this postcode."}),
                    headers=Headers.new([["Content-Type", "application/json"]]))
            ...
            if not rows:
                return Response.new(
                    json.dumps({"found": False,
                                "message": "No data found for this block."}),
                    headers=Headers.new([["Content-Type", "application/json"]]))

            row0   = rows[0]
            alarms = [
                {"rbe": _dv(r, "rbe"),
                 "rbe_display": _dv(r, "rbe_display") or _dv(r, "rbe"),
                 "status_date": _dv(r, "status_date")}
                for r in rows if _dv(r, "rbe")
            ]
            ...
            result = {
                "found":        True,
                "block_id":     f"{tc_p} {pfx_p} {block_p}",
                "tc": tc_p, "pfx": pfx_p, "block": block_p,
                "postcode":     _dv(row0, "postcode"),
                "address":      _dv(row0, "address") or _dv(row0, "full_add"),
                "lmd_device_id":_dv(row0, "lmd_device_id"),
                "alarms":       alarms,
                "lt": {
                    "town_council":  _dv(row0, "town_council"),
                    "full_add":      _dv(row0, "full_add"),
                    "lift_names_all":_dv(row0, "lift_names_all"),
                    "interface":     _dv(row0, "interface"),
                },
                "lmd_devices": row0.get("lmd_devices") or [],
                "block_detail_url": (
                    f"/block_detail?tc={_qenc(tc_p)}"
                    f"&pfx={_qenc(pfx_p)}&block={_qenc(block_p)}"
                ),
            }
            return Response.new(json.dumps(result),
                                headers=Headers.new([["Content-Type", "application/json"]]))
